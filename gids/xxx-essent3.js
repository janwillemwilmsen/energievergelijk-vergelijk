...
const page90 = await browser.newPage();
  ...
  //////ESSENT 
  await page90.goto("https://www.essent.nl/content/overessent/actueel/index.html");
  const contentessent = await page90.evaluate(() => {
    const data = [];

    const books = document.querySelectorAll(".headlinesets");
    books.forEach((book) => {
  ...
  // contentessent.forEach(item => console.log(item))
  // [contentessent].forEach(console.log, console);
  // [contentessent].forEach(console.log.bind(console));
 
  let firstItem = contentessent[0]
  let secondItem = contentessent[1]
  const thirdItem = contentessent[2]
  const fourthItem = contentessent[3]
  const fifthItem = contentessent[4]
...
fs.promises.writeFile(`${appRoot}/content/gids/3${bedrijfsnaam}` + '.md', `---
